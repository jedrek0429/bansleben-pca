...
def assert_stale_publish_temps_are_cleaned() -> None:
    with tempfile.TemporaryDirectory(prefix="pca-publish-temp-cleanup-") as directory:
        temp = Path(directory)
        dest = temp / "public_html"
        dest.mkdir()

        stale_stage = temp / ".public_html.pca-stage-stale"
        stale_backup = temp / ".public_html.pca-backup-stale"
        recent_stage = temp / ".public_html.pca-stage-recent"
        unrelated = temp / ".other.pca-stage-stale"
        for path in (stale_stage, stale_backup, recent_stage, unrelated):
            path.mkdir()
        ...
        if removed != 2:
            raise SystemExit(f"Expected two stale publish temp dirs to be removed, got {removed}")
        if stale_stage.exists() or stale_backup.exists():
            raise SystemExit("Stale publish transaction directories were not removed")
        if not recent_stage.exists():
            raise SystemExit("Recent publish transaction directory was removed")
        if not unrelated.exists():
            raise SystemExit("Cleanup removed a transaction directory for another destination")
...
def assert_queue_recovers_interrupted_jobs() -> None:
    with tempfile.TemporaryDirectory(prefix="pca-queue-recovery-") as directory:
        ...
        if worker.recover_interrupted_jobs(queue) != 1 or not (queue / "001.json").is_file():
...
def assert_dependencies_install_only_when_changed() -> None:
    with tempfile.TemporaryDirectory(prefix="pca-dependency-cache-") as directory:
        ...
        def record(command, cwd, log, check=True):
        ...
        try:
            ...
            worker.install_requirements(config, root, log)
            if len(calls) != 1:
                raise SystemExit("Unchanged requirements installed more than once")

            requirements.write_text("Jinja2==3.1.6\nPillow==11.3.0\n", encoding="utf-8")
            worker.install_requirements(config, root, log)
            if len(calls) != 2 or any("--upgrade" in command for command in calls):
                raise SystemExit("Dependency cache contract failed")
        finally:
            worker.run = real_run
...
def assert_production_uses_exact_sha_without_mutating_checkout() -> None:
    with tempfile.TemporaryDirectory(prefix="pca-exact-sha-") as directory:
        ...
        if ["git", "merge-base", "--is-ancestor", sha, "origin/main"] not in calls:
            raise SystemExit("Production preparation does not verify the queued SHA belongs to origin/main")
        if ["git", "worktree", "add", "--force", "--detach", str(target), sha] not in calls:
            raise SystemExit("Production worktree is not pinned to the queued SHA")
        if any(len(command) > 1 and command[0] == "git" and command[1] in {"checkout", "reset"} for command in calls):
...
def assert_contact_runtime_is_localized_data() -> None:
    ...
    if "function normalize_language" not in php or "'/^[a-z]{2}(-[A-Z]{2})?$/'" not in php:
        raise SystemExit("Contact language metadata is not shape-validated")
    if "? $lang : 'unknown'" not in php or "$lang = normalize_language" not in php:
        raise SystemExit("Malformed contact language metadata is not normalized")
    if "['en','fr','hr','pl']" in php or '["en", "fr", "hr", "pl"]' in php:
        raise SystemExit("Contact language validation reintroduced a hard-coded locale allowlist")
...
def main() -> None:
    ...
    assert_stale_publish_temps_are_cleaned()
