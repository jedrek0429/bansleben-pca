...
def main() -> None:
    ...
    formatting = TOOLS_DIR / "format_hyperlinks.py"
    build = TOOLS_DIR / "build.py"
    preview_assets = TOOLS_DIR / "rewrite_preview_asset_links.py"
    publish = TOOLS_DIR / "publish.py"

    for script in [validation, formatting, build, preview_assets, publish]:
    ...
    steps = [
        ("Validation", [python_bin, str(validation), "--root", str(root)]),
        ("Format Hyperlinks", [python_bin, str(formatting), "--root", str(root)]),
        ("Build", [python_bin, str(build), "--root", str(root)]),
    ]

    if args.lang_in_url:
        steps.append(("Preview Assets", [python_bin, str(preview_assets), "--dist", str(dist), "--url-prefix", os.environ["SITE_URL_PREFIX"]]))
    ...
    publish_command = [python_bin, str(publish), "--dist", str(dist), "--dest", str(dest)]
